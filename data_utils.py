...
def dataset_calculate_class_weights(
        dataset: Dataset, train_indices: list[int]
) -> dict:
    """
...
# TODO: either keep or delete this
# def compute_mean_pixel_value(dataset: Dataset):
#     loader = DataLoader(dataset, batch_size=32, shuffle=False)

#     pixel_sum = torch.zeros(3)  # Sum of non-black pixels per channel
#     pixel_count = torch.zeros(3)  # Number of valid pixels per channel

#     for images, _ in loader:  # Assuming dataset returns (image, label)
#         mask = (images != 0).all(dim=1, keepdim=True)  # Mask where pixels are NOT black (0,0,0)
        
#         # Apply mask and sum only valid pixels
#         valid_pixels = images * mask
#         pixel_sum += valid_pixels.sum(dim=(2, 3)).sum(dim=0)  # Sum across batch
        
#         # Count non-black pixels for each channel
#         pixel_count += mask.sum(dim=(2, 3)).sum(dim=0)

#     mean_pixel_value = pixel_sum / pixel_count.clamp(min=1)  # Avoid division by zero
#     return mean_pixel_value.view(3, 1, 1)  # Reshape to (C, 1, 1)
...
def dataset_print_stats(dataset: Dataset, dataset_name: str = None) -> None:
    ...
    print(f"\tNumber of samples: {total_count} (neg: {neg_count}, pos: "
          f"{pos_count})")
    print(f"\tNumber of classes: {len(classes)}")
    print(f"\tClass names: {classes}")
    print(f"\tClass distribution ratio (pos:neg): {class_ratio:.2f}")
    print(f"\tClass percentages: {pos_percentage:.2f}% pos, "
          f"{neg_percentage:.2f}% neg")
    print()
...
def pick_random_image_path(train_dir: str, class_name: str = None) -> str:
    ...
    if not os.path.exists(class_dir):
        raise ValueError(f"Class directory '{class_dir}' does not exist in the 
    ...
    if not image_files:
        raise ValueError(f"No images found in the class directory "
                         f"'{class_dir}'.")
...
def load_and_transform_image(
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Error loading image from path {image_path}: {e}"
        )
    ...
    if not isinstance(transformed_image, torch.Tensor):
        raise TypeError(f"Expected a torch.Tensor but got "
                        f"{type(transformed_image)}")
