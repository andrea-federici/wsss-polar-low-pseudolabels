...
from src.data.utils import load_and_transform_image, pick_random_image_path
...
optimizer = optim.Adam(
    filter(lambda p: p.requires_grad, torch_model.parameters()), lr=0.001
)

model = LitModel.load_from_checkpoint(
    "checkpoints/xception-xaug.ckpt",
    model=torch_model,
    criterion=criterion,
    optimizer=optimizer,
)
