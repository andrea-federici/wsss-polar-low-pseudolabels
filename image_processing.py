...
def normalize_image_by_statistics(
        image: torch.Tensor, mean, std, device: str = 'cpu'
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        device (str, optional): The device on which the computation should 
            be performed (e.g., "cpu" or "cuda"). Defaults to "cpu".
...
def unnormalize_image_by_statistics(
    ...
    This function reverses the normalization process applied to an image 
    tensor. It restores the original pixel values by scaling the tensor 
