...
def find_hurricane_mask(
    ...
    *,
    ...
    for step in tqdm(range(num_steps)):
        ...
        bin_loss = (m * (1 - m)).mean()  # encourages m→{0,1}
        l1 = (1 - m).abs().mean()
        # isotropic TV
        tv = (
            torch.abs(m[:, :, 1:, :] - m[:, :, :-1, :]).mean()
            + torch.abs(m[:, :, :, 1:] - m[:, :, :, :-1]).mean()
        )
        # 5b) Connectivity loss: penalize isolated pixels
        conn = connectivity_loss(1 - m, k=5)
        spread = spread_loss(m) * 1e-5
        loss = (
            lambda_l1 * l1
            + lambda_tv * tv
            + score
...
from src.data.image_processing import normalize_image_to_range, plot_to_pil_image
